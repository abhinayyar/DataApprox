...
class input_node
{
	...
	{
		weights.resize(next_layer_nodes,0.0);
		input=1.0;
		output=0.0;
		name= "abhi";
	}
...
class nn
{
	...
	nn(int _ni,int _no,int _nh,float _lr,int _iter);
	...
	float get_mse();
	float testprop(vector<pair<vector<float> , int> > test_set);
	void refresh_nodes();
	
};
nn::nn(int _ni,int _no,int _nh,float _lr,int _iter)
{
	ni=_ni+1; // for bias
	no=_no;
	nh=_nh;
	...
	for(int i=0;i<ni;i++)
	{
		...
		for(int k=0;k<i_nodes.back()->weights.size();k++)
		{
			cout<<"val "<<i_nodes.back()->weights[k]<<endl;
		}
		*/
...
float nn::testprop(vector<pair<vector<float> , int> > test_set)
{
	float pass=0;
	...
	for(pair<vector<float>,int> input : test_set)
	{
		...
		//cout << (target+1)*4<<"\t" << " ----> " << "\t"<<(index+1)*4<<endl;
		//cout << target << endl;	
...
void nn::backprop(vector<pair<vector<float> , int> > train_set)
{
	...
	while(1)
	{
		...
		if(error_epoch<0.000006 || iterations > iter)
		{
			cout << "Reached break cond [ error ] [ iterations] \n";
			cout<<error_epoch<<"\t"<<iterations<<endl;
		 	break;
		 }
		iterations++;
